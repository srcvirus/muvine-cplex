...
int main(int argc, char* argv[]) {
  ...
  unique_ptr<DWDMGraph> dwdm_topology(
      InitializeDWDMGraphFromFile(config->dwdm_topology_file.c_str())
          .release());
  ...
  FILE* sol_time_file =
      fopen((config->vn_topology_file + ".time").c_str(), "w");
  fprintf(sol_time_file, "%llu.%llu\n", elapsed_time / 1000000000LL,
          elapsed_time % 1000000000LL);
  fclose(sol_time_file);

  vne_sbuilder.PrintSolutionStatus(
      (config->vn_topology_file + ".status").c_str());
  if (success) {
    vne_sbuilder.PrintCost((config->vn_topology_file + ".cost").c_str());
    vne_sbuilder.PrintVNodeMapping(
        (config->vn_topology_file + ".nmap").c_str());
    vne_sbuilder.PrintVLinkMapping(
