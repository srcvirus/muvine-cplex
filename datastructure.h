...
class IPEdgeEndpoint : public EdgeEndpoint {
  public:
    IPEdgeEndpoint() : EdgeEndpoint(), order_(NIL), bandwidth_(0), 
      residual_bandwidth_(0), cost_(INF) {}
    IPEdgeEndpoint(int n, int o, long bw, int c):
      EdgeEndpoint(n), order_(o), bandwidth_(bw),
      residual_bandwidth_(bw), cost_(c) {}
...
class OTNEdgeEndpoint : public EdgeEndpoint {
  public:
    ...
    std::vector<std::vector<int>>& module_res_cap() { 
      return module_res_cap_; 
    }
  private:
 ...
 public:
  ...
  void AddEdge(int u, int v, long bw, int cost) {
    ...
    adj_list_->at(u).push_back(
        IPEdgeEndpoint(v, order, bw, cost));
    adj_list_->at(v).push_back(
        IPEdgeEndpoint(u, order, bw, cost));
    ++edge_count_;
...
class OTNGraph {
 public:
  ...
  long GetModuleResidualCapacity(
      int u, int v, int module_type, int module_instance) const {
    auto& neighbors = adj_list_->at(u);
    for (auto& end_point : neighbors) {
...
class DWDMGraph {
 public:
  ...
  bool AddEdge(int u, int v, const std::vector<bool>& lambda_mask, int cost) {
    ...
    adj_list_->at(u).push_back(
        DWDMEdgeEndpoint(v, lambda_mask, cost));
    adj_list_->at(v).push_back(
        DWDMEdgeEndpoint(u, lambda_mask, cost));
    ++edge_count_;
    node_count_ = adj_list_->size();
    return true;
  }
  ...
  bool IsWavelengthAvailable(int u, int v, int lambda) {
    ...
    for (auto& end_point : neighbors) {
      if (end_point.node_id() == v)
        return end_point.is_lambda_free()[lambda];
    }
    return false;
  }
  ...
  std::vector<bool>& GetWavelengthMask(int u, int v) {
    ...
    for (auto& end_point : neighbors) {
      if (end_point.node_id() == v)
        return end_point.is_lambda_free();
    }
  }
...
typedef struct dwdm_edge_t {
  ...
  dwdm_edge_t(int f, int s, int l):
    first(f), second(s), lambda(l){}
